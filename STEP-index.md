...
| STEP-5 | Storefront feature | Raul Angel | In progress | `quasar-disney-mobile-app` | Dark-theme home: header, four-tab bar with `ComingSoon` placeholders, config-driven carousel (continue-watching, standard portrait, 3:4 hero stand-in), pagination wrappers, hero + progress composition, silent CW reload, and card tap → title alert. Tests: composition, `loadMore`/`hasMore`, unknown-variant drop+warn, and both paging axes (T1/T2). Depends on STEP-2 and STEP-3 merged. PLAN + prompts in `Upcoming Prompts/`. Branch `step-0005-storefront-feature`. |
